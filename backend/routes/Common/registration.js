...
function registrationRouter(connection) {
    ...
    router.post('/', async(req, res) => {
        ...
        if (username == null || username.length > 65 || username.length < 1) {
            return res.status(400).send({ error: 'Username needs to be at least 1 character and no more than 64' });
        }

        if (password == null || password.length < 7 || password.length > 25) {
        ...
        await Player.findOne({
            ...
            if (player) {
                ...
                newPlayer.save().then(player => res.json({ msg: "success" })).catch(error => console.log(error))
            }
        });
